    ...
    def calculate_perceptions(self):
        if self.move_type == 'out_invoice' or self.move_type == 'out_refund':
            ...
            if self.invoice_line_ids:
                if self.partner_id:
                    if self.partner_id.alicuota_per_misiones > 0 or self.partner_id.alicuota_per_misiones_3 or self.partner_id.alicuota_per_misiones_1:
                        #Busco el impuesto a utilizar para la percepcion
                        imp_per_sircar_misiones = self.company_id.tax_per_sircar_misiones
                        if len(imp_per_sircar_misiones) == 0:
                        ...
                        imp_per_sircar_misiones.amount = self.partner_id.get_amount_alicuot_misiones('per',self.invoice_date)
                        ...
                        for iline in self.invoice_line_ids:
                            ...
                            for tax in iline.tax_ids:
                                if str(imp_per_sircar_misiones.id) == str(tax.id)[-2:]:
                                    _tiene_precepcion = 1
                            if not _tiene_precepcion and imp_per_sircar_misiones.amount > 0:
                                iline.write({'tax_ids': [(4, imp_per_sircar_misiones.id)]})
